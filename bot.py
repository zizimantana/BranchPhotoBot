...
def make_webapp_auth_url(
    telegram_id: int
):
    # Short-lived signed token tied to this Telegram account.
...
def employee_keyboard(
    ...
    webapp_auth_url = make_webapp_auth_url(
        telegram_id
    )
...
async def start_handler(
    ...
    await message.answer(
        ...
        reply_markup=employee_keyboard(user_id),
        parse_mode="HTML"
    )
...
def request_user(
    ...
    # Preferred: Telegram's signed initData.
    ...
    # Fallback: short-lived signed URL generated by the bot.
    try:

        raw_uid = request.query.get(
            "uid",
            ""
        )

        raw_ts = request.query.get(
            "ts",
            ""
        )

        received_sig = request.query.get(
            "sig",
            ""
        )
        ...
        # URL token is valid for 10 minutes.
        if (
            not received_sig
            or abs(now_ts - ts) > 600
        ):
            return None
